...
from .model.lstm_ppo import PPOAgent as LSTMPPOAgent
from .model.ppo import PPOAgent
from .environment.ghfr_gym_rd import SINDyGYM
from .model.mpi_utils import proc_id, mpi_avg, mpi_sum
from .environment.model_sindy_gfhr import sindyDx
from mpi4py import MPI




def train(args, load_weights=False): 
    agent_config = {
    ...
    if os.environ.get("BOOTSTRAP_FOR_LONG_EPISODE") == "True":
        print("Bootstrapping value function to learn from LONG episodes...")
    else:
        print("Using collected trajectories for calculation...")

    if load_weights:
        print("Loading args and agent_config from the saved model... ")
        MODEL_PATH = args.saved_model_path
        with open(MODEL_PATH + "config", "rb") as file:
            config = pickle.load(file)
        args = config["args"] # overwriting args
        agent_config = config["agent"] # overwriting agent_config
    ...
    if proc_id() == 0:
        ...
        if load_weights:
            expid = args.expid + "_contd" 
            folder = expid + "_" + str(np.random.randint(0, 100)) + date_time
        else:
    ...
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
        initial_learning_rate=args.lam_lr, decay_steps=5, decay_rate=0.96
    )
    ...
    if load_weights:
        print("Loading pretrained model weights, Lagrangians, and optimizer states...")
        SAVED_W = "model_weights_epoch_360"
    ...
    for epoch in range(epochs):
        ...
        for t in tqdm(range(steps_per_epoch)):
            ...
            o2, rew_vec, d, f = env.step(
                a[0].numpy(), proc_id()
            )  # return failure flag. If env fails f=True. 
            ep_ret += rew_vec
            ep_len += 1
            if f:
            ...
            if os.environ.get("BOOTSTRAP_FOR_LONG_EPISODE") == "True":
                '''
                If bootstrapping, check if it is the last step of every 2250 steps
                If so, add the value network prediction as the value for the last time.
                '''
            ...
            if so, return the termination value
            if it has not reached the end of the episode
            boostrap
            if it did, use rew_vec.
            '''
            if terminal or (t == steps_per_epoch - 1):
                ...
                # if f: # should break steps_per_epoch loop not epoch loop
                #     break
        ...
        mean_return = mpi_sum(mean_return)/mpi_sum(~inputs['failure'])
        mean_return = np.asarray(mean_return)

        return_std = np.std(epoch_ep_ret, axis=0)
        return_std = np.asarray(mpi_sum(return_std) / mpi_sum(~inputs['failure']))


        mean_entropy = np.mean(epoch_entropy, axis=0)
        mean_entropy = mpi_sum(mean_entropy) / mpi_sum(~inputs['failure'])
        ...
        for i in range(5):
            ...
            inputs_refac = dict()
            inputs_refac['failure'] = inputs['failure']
            if args.lstm_actor:
        ...
        logger["ep_ret_std"].append(return_std)
