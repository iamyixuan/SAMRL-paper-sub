...
default_config = {
    "steps_per_epoch": 20000,  # 1600, cartpole
    "max_ep_len": 20000,  # 200,
    "epochs": 400,  # 20,
    "gamma": 0.99,
    "clip_ratio": 0.1,
    "pi_lr": 1e-4,
...
class PPOAgent:
    ...
    @tf.function  # (because we are doing mpi op over numpy arrays)
    ...
    def train_pi_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            target_logp, entropy = self.policy_model.policy_train(
                obs_ph=states, cons_bounds=cons_bounds, action_ph=actions, training=True
            )
    ...
    @tf.function
    def train_v_step(self, inputs):
        ret = inputs["ret"]
        states = inputs["state"]
        with tf.GradientTape() as tape:
            v = tf.squeeze(self.value_model(states, training=True), axis=2)
    ...
    def update(self, inputs):
        ...
        for i in range(self.train_pi_iters):
            loss_pi, approx_kl, pi_gradients = self.train_pi_step(inputs)
            mask_pi_gradients = []
            if inputs['failure']:
                for v in pi_gradients:
                    mask_pi_gradients.append(tf.zeros_like(v))
                approx_kl = tf.zeros_like(approx_kl)
                loss_pi = tf.zeros_like(loss_pi)
                pi_gradients = mask_pi_gradients
            np_gradients = []
            for grad in pi_gradients:
                np_grad = grad.numpy()
                np_grad = mpi_sum(np_grad) / mpi_sum(int(~inputs['failure']))
                np_gradients.append(np_grad)
            self.optimizer_pi.apply_gradients(
                zip(np_gradients, self.policy_model.trainable_variables)
            )
            approx_kl = mpi_sum(approx_kl) / mpi_sum(~inputs['failure'])
            loss_pi = mpi_sum(loss_pi) / mpi_sum(~inputs['failure'])
        ...
        for _ in range(self.train_v_iters):
            mse, v_gradients = self.train_v_step(inputs)
            np_gradients = []
            mask_v_gradients = []
            if inputs['failure']:
                for v in v_gradients:
                    mask_v_gradients.append(tf.zeros_like(v))
                mse = tf.zeros_like(mse)
                v_gradients = mask_v_gradients
            for grad in v_gradients:
                np_grad = grad.numpy()
                np_grad = mpi_sum(np_grad) / mpi_sum(~inputs['failure'])
                np_gradients.append(np_grad)
            self.optimizer_v.apply_gradients(
                zip(np_gradients, self.value_model.trainable_variables)
            )
            mse = mpi_sum(mse) / mpi_sum(~inputs['failure'])
            loss_v_list_mse.append(mse)
        return (
            np.mean(loss_pi_list[-1]),
            np.mean(approx_kl_list[-1]),
            np.mean(loss_v_list_mse[-1]),
        )

    def output_actions(self, state, cons_bounds=None, deterministic=False):
        action, logp_action, entropy = self.policy_model.policy_act(
            state, cons_bounds, training=False, deterministic=deterministic
        )
        return action, tf.squeeze(logp_action), entropy
    ...
    def build_agent(
        ...
        self.policy_model.build(input_shape=[None, self.obs_dim])
        ...
        self.value_model.build(input_shape=[None, self.obs_dim])
